...
$query = "select orders.*,date_format(orders.date,'%Y-%m-%d') as dat,  order_status.status as status_name,
          ...
          towns.name as town_name,
            ...
            ) + if(total_price > 500000 ,( (ceil(total_price/500000)-1) * ".$config['addOnOver500']." ),0) as dev_price,
            total_price -
              (if(orders.city_id = 1,
                  if(client_dev_price.price is null,(".$config['dev_b']." - discount),(client_dev_price.price - discount)),
                  if(client_dev_price.price is null,(".$config['dev_o']." - discount),(client_dev_price.price - discount))
                 ) + if(total_price > 500000 ,( (ceil(total_price/500000)-1) * ".$config['addOnOver500']." ),0)
             ) as client_price
          from orders
