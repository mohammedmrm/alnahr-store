...
try{
  ...
  $query = "select orders.*, date_format(orders.date,'%Y-%m-%d') as date,
            ...
            ";
  $where = "where";
  $filter = " and orders.confirm = 1 and orders.mandop_id = ".$userid;
  if($driver >= 1){
...
echo json_encode(array("code"=>200,"message"=>$msg,"success"=>$success,"data"=>$data,'total'=>$total,"pages"=>$pages,"page"=>$page));
?>